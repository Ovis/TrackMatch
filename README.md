...
- Chromaprintによる音声フィンガープリントを利用した重複候補の検出
- A/B同期試聴と、解析された同期位置の手動微調整
- ラウドネス、True Peak、ダイナミクス、高域成分、クリッピング疑いなどの品質比較情報
- ジャンルや一致度による候補の絞り込み
- 3ファイル以上を含む重複グループと、残すファイルの管理
- 解析済み情報とレビュー結果を再利用する増分スキャン
- 見つからなくなったファイルを含むTrack管理
- 安全確認を行ったうえでの不要ファイル整理

品質解析は、どちらのファイルが高品質かを自動的に決定するものではありません。測定可能な特徴や注意点を提示し、最終的な判断は利用者が行います。
...
4. 「スキャン・分析」を実行します。TrackMatchが音声ファイルを調べ、重複している可能性のある組み合わせを探します。
5. 見つかった候補を確認します。必要に応じてA/B同期試聴、品質比較情報、ジャンルフィルターなどを利用できます。
6. 次のいずれかを選択してレビュー結果を記録します。
   - `重複ではない` — 2つのファイルを重複ではないと判定します。
   - `重複 / Aを残す` — 重複と判定し、Aを残すファイルとして選択します。
   - `重複 / Bを残す` — 重複と判定し、Bを残すファイルとして選択します。
7. 重複グループと、残すファイルを確認します。
8. 不要なファイルを整理する場合は、ごみ箱フォルダーを設定して「ごみ箱へ移動」を実行します。

重複と判定しただけでは、ファイルが削除・移動されることはありません。この時点ではTrackMatch上で重複として記録されるだけです。実際のファイルは、後から「ごみ箱へ移動」を実行したときに初めて移動されます。
...
重複と判定しただけでは、ファイルが削除・移動されることはありません。削除候補のファイルは、「ごみ箱へ移動」を実行したときに、設定したごみ箱フォルダーへ移動されます。実行前に、対象となるファイルと移動先を十分に確認してください。

移動先に同じ名前のファイルがある場合は上書きしません。また、重複の判定に矛盾があるなど、安全に整理できない状態ではファイルの移動を行いません。

ファイルをごみ箱へ移動した後、TrackMatch側の記録更新に失敗した場合は、できる限りファイルを元の場所へ戻します。ただし、予期しない問題によるファイルの消失を完全に防げるわけではありません。
