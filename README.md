...
重複と判定しただけではファイルが削除・移動されることはありませんが、実際に「ごみ箱へ移動」を実行する前には、対象となるファイルと移動先を十分に確認してください。
...
重複と判定しただけでは、ファイルが削除・移動されることはありません。削除候補のファイルは、「ごみ箱へ移動」を実行したときに、設定したごみ箱フォルダーへ移動されます。
