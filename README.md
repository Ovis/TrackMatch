...
再スキャン中の「処理済み件数 / 総件数」の総件数には、前回のスキャンでTrackMatchが記録した対象フォルダー内のファイル数を使用します。件数表示のためだけにNASや大規模な音楽フォルダーを事前走査するとファイル列挙が二重になり、スキャン開始と完了が遅くなるためです。前回のスキャン後にファイルを追加・削除した場合は表示上の総件数と今回実際に見つかる件数が一時的に異なることがありますが、スキャン完了後の記録は今回の結果に更新されます。
