...
public sealed class CandidateReviewPersistenceTests : IAsyncLifetime
{
    ...
    public async Task ReviewRepository_PersistsNotDuplicateReview()
    {
        ...
        await repository.SaveAsync(
            new CandidateReview(pair, CandidateReviewDecision.NotDuplicate, null, "別アレンジ"),
            TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task ReviewRepository_PersistsConfirmedDuplicateWithPreferredTrack()
    {
        var (trackIdA, trackIdB) = await CreateTracksAsync();
        var repository = new SqliteCandidateReviewRepository(_database);
        ...
        await repository.SaveAsync(
            new CandidateReview(pair, CandidateReviewDecision.ConfirmedDuplicate, trackIdA, "重複として確認"),
            TestContext.Current.CancellationToken);
        ...
        Assert.Equal(trackIdA, review.PreferredTrackId);
    ...
    public async Task ReviewRepository_ReplacingDecisionArchivesPreviousGlobalVerdict()
    {
        ...
        var pair = CandidatePairKey.Create(trackIdA, trackIdB);
        await repository.SaveAsync(
            new CandidateReview(pair, CandidateReviewDecision.ConfirmedDuplicate, trackIdA, "最初の判定"),
            TestContext.Current.CancellationToken);

        await repository.SaveAsync(
            new CandidateReview(pair, CandidateReviewDecision.NotDuplicate, null, "再確認"),
            TestContext.Current.CancellationToken);
        ...
        command.CommandText = "SELECT Decision, PreferredTrackId, ChangeKind FROM CandidateReviewHistory WHERE TrackIdA = $a AND TrackIdB = $b;";
        ...
        Assert.Equal(trackIdA, reader.GetInt64(1));
        Assert.Equal("UserChanged", reader.GetString(2));
    }
