    ...
    IDuplicateGroupRepository groupRepository)
{
    ...
    public async Task SaveReviewAsync(long libraryId, CandidateReview review, CancellationToken cancellationToken = default)
    {
        ...
        var existing = allReviews.SingleOrDefault(item => item.Pair == review.Pair);
        ...
        try
        {
            ...
            await ApplyPreferredTrackAsync(libraryId, review, CancellationToken.None);
        }
        catch
        {
    ...
    /// <summary>
    /// 旧呼び出し規約との移行境界。ConfirmedDuplicateのkeepTrackIdはHuman VerdictのPreferredTrackIdと一致しなければならない。
    /// </summary>
    ...
    {
        if (review.Decision == CandidateReviewDecision.ConfirmedDuplicate && review.PreferredTrackId != keepTrackId)
        {
            throw new ArgumentException("Keep TrackとHuman VerdictのPreferredTrackIdが一致していません。", nameof(keepTrackId));
        }
        if (review.Decision == CandidateReviewDecision.NotDuplicate && keepTrackId is not null)
        {
            throw new ArgumentException("NotDuplicateではKeep Trackを指定できません。", nameof(keepTrackId));
        }
        return SaveReviewAsync(libraryId, review, cancellationToken);
    }
    ...
    private async Task ApplyPreferredTrackAsync(long libraryId, CandidateReview review, CancellationToken cancellationToken)
    {
        if (review.Decision != CandidateReviewDecision.ConfirmedDuplicate || review.PreferredTrackId is not { } preferredTrackId) return;
        ...
        if (group is null)
        {
            ...
            group = await groupRepository.GetByTrackIdAsync(review.Pair.TrackIdA, libraryId, cancellationToken)
                ?? throw new InvalidOperationException("保存済みHuman VerdictからGlobal Duplicate Groupを解決できませんでした。");
        }

        // Preferred Trackの伝播が一意な場合だけ派生Keepへ反映する。既存Conflictは人手確認なしに解消しない。
        if (group.KeepStatus == DuplicateGroupKeepStatus.Conflict) return;
        if (group.KeepStatus == DuplicateGroupKeepStatus.Selected && group.KeepTrackId == preferredTrackId) return;
        await groupRepository.SetKeepAsync(libraryId, group.Id, preferredTrackId, "HumanVerdict", cancellationToken);
    }
    ...
    private async Task<IReadOnlyList<CandidateReview>> GetActiveGlobalReviewsAsync(IReadOnlyList<CandidateReview> allReviews, CancellationToken cancellationToken)
    {
    ...
    private async Task EnsurePairBelongsToLibraryAsync(long libraryId, CandidatePairKey pair, bool requireActiveTracks, CancellationToken cancellationToken)
    {
        ...
        if (!await trackLookupRepository.IsInLibraryAsync(pair.TrackIdA, libraryId, cancellationToken) || !await trackLookupRepository.IsInLibraryAsync(pair.TrackIdB, libraryId, cancellationToken)) throw new InvalidOperationException("現在LibraryのMembership外Track同士を通常レビューとして扱うことはできません。");
    }
    ...
    private async Task<StoredTrack?> GetTrackCachedAsync(long trackId, IDictionary<long, StoredTrack?> cache, CancellationToken cancellationToken)
    {
        ...
        var track = await trackLookupRepository.GetByIdAsync(trackId, cancellationToken); cache[trackId] = track; return track;
    }
    ...
    private static bool HasTopologyChanged(IReadOnlyCollection<DuplicateGroupRebuildItem> rebuild, IReadOnlyCollection<GlobalDuplicateGroup> existingGroups)
    {
        ...
        return rebuild.Any(plan => plan.ExistingGroupId is not { } groupId || !existingById.TryGetValue(groupId, out var existing) || !plan.TrackIds.Order().SequenceEqual(existing.TrackIds.Order()));
    }
}
