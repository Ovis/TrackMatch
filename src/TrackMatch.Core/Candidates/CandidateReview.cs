...
public sealed partial record CandidateReview(
    ...
    string? Note)
{
    /// <summary>永続化前にHuman Verdictとして有効な値であることを検証する。</summary>
    public void Validate()
    {
        ...
        {
            if (PreferredTrackId is null) throw new InvalidOperationException("ConfirmedDuplicateにはPreferredTrackIdが必要です。");
            if (PreferredTrackId != Pair.TrackIdA && PreferredTrackId != Pair.TrackIdB) throw new InvalidOperationException("PreferredTrackIdは判定対象PairのTrackである必要があります。");
        }
