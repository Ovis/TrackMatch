...
/// <summary>
/// Library CRUDとRoot管理を集約するModal Dialog。
/// </summary>
public partial class LibraryManagementDialog : Window
{
    ...
    public LibraryManagementDialog(LibraryManagementService service, long? selectedLibraryId)
    {
    ...
    private async void AddRoot_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            await _service.AddRootAsync(library.Id, dialog.FolderName);
    ...
    protected override void OnClosing(System.ComponentModel.CancelEventArgs e)
    {
        ...
        // これによりタイトルバーの×でも「保存 / 破棄 / キャンセル」を同じ意味で提供できる。
        e.Cancel = true;
        base.OnClosing(e);
        if (!_closingConfirmationActive)
        {
            _closingConfirmationActive = true;
    ...
    private async Task ConfirmAndCloseAsync()
    {
        ...
        {
            _closingConfirmationActive = false;
        }
    }
}
