...
public sealed class MainWindowQualityAnalysisController : IDisposable
{
    ...
    public void Restart()
    {
        ...
        if (libraryId is null) { _setStatusText("音質解析: ライブラリ未選択"); return; }
    ...
    public async Task ReanalyzeSelectedAsync()
    {
        ...
        try
        {
            ...
            var progress = new Progress<TrackQualityAnalysisProgress>(value => _setStatusText($"音質解析中 {value.CompletedCount} / {value.TotalCount}"));
            await coordinator.RunAsync([new TrackQualityAnalysisRequest(selected.TrackIdA, selected.Row.PathA), new TrackQualityAnalysisRequest(selected.TrackIdB, selected.Row.PathB)], progress, cancellation.Token);
        ...
        catch (OperationCanceledException) { _setStatusText("音質解析: キャンセルしました"); }
        finally
        {
    ...
    private async Task RunSessionAsync(long libraryId, CancellationToken cancellationToken)
    {
        ...
        var rows = (await new SqliteCandidateReviewReportRepository(database).GetAsync(libraryId, cancellationToken))
            ...
            .ToArray();
        if (rows.Length == 0) { _setStatusText("音質解析: 対象候補なし"); return; }
        ...
        foreach (var row in orderedRows)
        {
            ...
            if (FindVisibleCandidate(row.TrackIdA, row.TrackIdB) is { } item) await RefreshPresentationAsync(item, trackRepository, candidateRepository, cancellationToken, result);
        }
        await RefreshVisiblePresentationsAsync(trackRepository, candidateRepository, cancellationToken);
        _setStatusText($"音質解析完了 {requests.Count}曲 / 比較 {orderedRows.Count}件");
    }

    private async Task TryAnalyzeAndRefreshAsync(CandidateReviewItemViewModel item, CandidateQualityAnalysisService candidateService, SqliteTrackQualityAnalysisRepository trackRepository, SqliteCandidateQualityComparisonRepository candidateRepository, CancellationToken cancellationToken)
    {
    ...
    private async Task RefreshVisiblePresentationsAsync(SqliteTrackQualityAnalysisRepository trackRepository, SqliteCandidateQualityComparisonRepository candidateRepository, CancellationToken cancellationToken)
    {
    ...
    private static async Task RefreshPresentationAsync(CandidateReviewItemViewModel item, SqliteTrackQualityAnalysisRepository trackRepository, SqliteCandidateQualityComparisonRepository candidateRepository, CancellationToken cancellationToken, CandidateQualityComparison? knownComparison = null)
    {
    ...
    private CandidateReviewItemViewModel? FindVisibleCandidate(long trackIdA, long trackIdB) => _viewModel.Candidates.FirstOrDefault(item => item.TrackIdA == trackIdA && item.TrackIdB == trackIdB);

    private static CandidateQualityAnalysisService CreateCandidateService(SqliteTrackQualityAnalysisRepository trackRepository, SqliteCandidateQualityComparisonRepository candidateRepository)
    ...
    private static CandidateQualityAnalysisRequest CreateRequest(CandidateReviewReportRow row)
        => new(new CandidateComparison(row.TrackIdA, row.TrackIdB, row.Similarity, 0, row.BestOffset, 0, row.MatchedDuration, row.CoverageA, row.CoverageB, row.DurationRatio), row.PathA, row.PathB);

    private async Task StopAndWaitAsync()
    {
        var task = _sessionTask; Stop(); if (task is null) return;
    ...
    {
        try { await task; }
        catch (OperationCanceledException) { if (!_disposed) _setStatusText("音質解析: キャンセルしました"); }
        catch (Exception ex) { if (!_disposed) _setStatusText($"音質解析失敗: {ex.Message}"); }
        finally
        {
            if (ReferenceEquals(_cancellation, cancellation))
            {
                _cancellation = null; _sessionTask = null; _trackCoordinator = null; cancellation.Dispose();
            }
        }
    }
    ...
    private void ViewModel_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        {
            // 下限は表示だけでなく解析対象も定義するため、進行中のセッションを止めて新しい対象集合で再開する。
            Restart();
        }
    }
